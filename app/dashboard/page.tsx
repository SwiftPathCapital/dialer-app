...
export default function DashboardPage() {
  const { agent, agentLoading, activeCall } = useSoftphone()
  ...
  // When a call becomes active, look up the remote number in leads + history
  useEffect(() => {
    const number = activeCall?.remoteNumber
    if (!number) { setLead(null); setCallHistory([]); return }

    const digits = number.replace(/\D/g, '')

    fetch(`/api/leads?phone=${encodeURIComponent(digits)}&limit=1`)
      .then(r => r.json())
      .then(d => setLead(Array.isArray(d) && d.length > 0 ? d[0] : null))
      .catch(() => {})
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-wrap gap-6">
        ...
        {/* Right: lead info + history — only shown during a call */}
        {activeCall && (lead || callHistory.length > 0) && (
            ...
            {lead && (
              <div className="bg-gray-800 rounded-xl border border-gray-700 p-5 space-y-3">
                ...
                <div className="border-t border-gray-700 pt-3 space-y-2">
                  ...
                  {lead.status && <Row label="Status" value={lead.status} />}
