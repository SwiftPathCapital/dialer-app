...
export async function GET(req: NextRequest) {
  ...
  const { data: convos } = await db
    ...
    .select('id, contact_number, our_number')
  ...
  const [{ data: messages }, { data: calls }, { data: leads }] = await Promise.all([
    ...
    db.from('dialer_calls')
      ...
      .limit(500),
    db.from('leads').select('id, first_name, last_name, name, company_name, phone, email, status, lead_type_label').ilike('phone', `%${key}%`).limit(1),
  ])
